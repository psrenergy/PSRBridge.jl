...
function build_existing(date_time::DateTime)
    return Dates.month(date_time) % 2 == 0
end

function build_shutdown_cost(i::Integer)
    return Float64(i)
end

function build_initial_volume(i::Integer)
    return Float64(i)
end

function build_max_startups(i::Integer)
    return i
end

function build_max_generation(date_time::DateTime)
    return Float64(Dates.month(date_time))
end

function build_has_commitment(i::Integer)
    return i % 2 == 0
end
...
function build_database(path::AbstractString)
    ...
    for i in 1:THERMAL_PLANT_SIZE
        add_thermal_plant!(
            ...
            parameters = DataFrame(;
                ...
                existing = [build_existing(date_time) ? 1 : 0 for date_time in DATE_TIMES],
                max_generation = [build_max_generation(date_time) for date_time in DATE_TIMES],
            ),
        )
    end
    ...
    for i in 1:HYDRO_PLANT_SIZE
        add_hydro_plant!(db;
            ...
            has_commitment = build_has_commitment(i) ? 1 : 0,
            parameters = DataFrame(;
                date_time = DATE_TIMES,
                existing = [build_existing(date_time) ? 1 : 0 for date_time in DATE_TIMES],
                max_generation = [build_max_generation(date_time) for date_time in DATE_TIMES],
